...
func _append_command(text: String, on: Node) -> void:
    ...
    cmd_label.text = "[/%s]$ %s" % [on.name, text]
    shell_history.add_child(cmd_label)
    shell_history.move_child(cmd_label, -2) # Move to the second last position
...
func _on_command_execution_finished(result: Variant, show_as: String) -> void:
    match show_as:
        "text":
            ...
            shell_history.move_child(result_label, -2) # Move to the second last position
        "list":
            ...
            shell_history.move_child(list_container, -2)
        "tree":
            pass
